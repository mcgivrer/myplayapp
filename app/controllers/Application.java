...
public class Application extends Controller {
	...
	public static void index() {
		...
		if (user != null) {
			...
			if(platform!=null){
				games = Game.find("select g from Game g "
									+"where "
									+"g.publish=true "
									+"and g.author = ? "
									+"and g.platform = ? "
									+"order by platform asc, title asc", user,platform).fetch();
			}else{
				games = Game.find("select g from Game g "
									+"where "
									+"g.publish=true "
	...
	public static void filterByPlatform(String platform) {
		...
		if(platform.equals("*")){
			...
			games = Game.find("select g from Game g where g.publish=true and g.author=? order by title asc", user).fetch();
		}else{
			session.put("filterPlatform", platform);
			games = Game.find("select g from Game g where g.publish=true and g.author=? and g.platform=? order by title asc", user, platform).fetch();
		}
